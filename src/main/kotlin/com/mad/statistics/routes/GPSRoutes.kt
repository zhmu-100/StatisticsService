...
fun Application.configureGPSRoutes() {
  ...
  routing {
    route("/api/statistics") {
      ...
      get("/gps") {
        val exerciseId =
            call.request.queryParameters["exercise_id"]
                ?: return@get call.respondText(
                    "Missing exercise_id parameter", status = HttpStatusCode.BadRequest)
      ...
      post("/gps") {
        try {
          ...
          call.respondText(
              "Error uploading GPS data: ${e.message}", status = HttpStatusCode.InternalServerError)
        }
      }
    }
  }
