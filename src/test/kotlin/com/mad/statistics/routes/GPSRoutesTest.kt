...
class GPSRoutesTest {
  ...
  private fun testModules(service: GPSService = mockService, logger: LoggerClient = mockLogger) =
      listOf(module { single<GPSService> { service } }, module { single<LoggerClient> { logger } })
  ...
  private val testPosition =
      GPSPosition(
          ...
          speed = 5.5,
          accuracy = 3.0)
  private val testData = GPSData(meta = testMeta, positions = listOf(testPosition))
  ...
  fun `POST gps with valid body returns OK`() {
    ...
    withTestApplication({
      ...
      val call =
          handleRequest(HttpMethod.Post, "/api/statistics/gps") {
