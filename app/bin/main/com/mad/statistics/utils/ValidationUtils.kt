...
object ValidationUtils {
    ...
    fun validateGPSPositions(positions: List<GPSPosition>) {
        ...
        positions.forEach { position ->
            require(position.latitude >= -90 && position.latitude <= 90) { "Latitude must be between -90 and 90" }
            require(position.longitude >= -180 && position.longitude <= 180) { "Longitude must be between -180 and 180" }
