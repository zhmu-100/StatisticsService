...
class GPSRepository(clickHouseServiceClient: ClickHouseServiceClient) :
    RepositoryBase(clickHouseServiceClient) {

  fun saveGPSData(gpsData: GPSData) {
    runBlocking {
      val dataToInsert =
          gpsData.positions.map { position ->
            buildJsonObject {
  ...
  fun getGPSDataByExerciseId(exerciseId: String): List<GPSData> {
    return try {
          ...
          result
              .mapNotNull { element ->
                try {
                  ...
                  val id =
                      element["id"]?.jsonPrimitive?.contentOrNull ?: UUID.randomUUID().toString()
                  ...
                  val positionTimestampStr =
                      element["position_timestamp"]?.jsonPrimitive?.contentOrNull ?: ""
                  ...
                  Triple(
                      exerciseIdFromDb,
                      ExerciseMetadata(
                          id = id, exerciseId = exerciseIdFromDb, timestamp = timestamp),
                      GPSPosition(
      ...
      groupedResults.map { (exerciseId, triples) ->
        ...
        GPSData(meta = metadata, positions = positions)
      }
    } catch (e: Exception) {
