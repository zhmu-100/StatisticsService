...
class ValidationUtilsTest {
  ...
  fun `validateExerciseMetadata passes for valid metadata`() {
    val metadata =
        ExerciseMetadata(id = "meta1", exerciseId = "exercise1", timestamp = dummyTimestamp)
    ValidationUtils.validateExerciseMetadata(metadata)
  }

  @Test
  fun `validateExerciseMetadata throws for blank id`() {
    val metadata = ExerciseMetadata(id = "", exerciseId = "exercise1", timestamp = dummyTimestamp)
    val ex =
  ...
  fun `validateExerciseMetadata throws for blank exerciseId`() {
    val metadata = ExerciseMetadata(id = "meta1", exerciseId = "", timestamp = dummyTimestamp)
    val ex =
        assertFailsWith<IllegalArgumentException> {
          ValidationUtils.validateExerciseMetadata(metadata)
        }
  ...
  fun `validateUserMetadata passes for valid metadata`() {
    val metadata = UserMetadata(id = "userMeta1", userId = "user1", timestamp = dummyTimestamp)
    ValidationUtils.validateUserMetadata(metadata)
  }

  @Test
  fun `validateUserMetadata throws for blank id`() {
    val metadata = UserMetadata(id = "", userId = "user1", timestamp = dummyTimestamp)
    val ex =
        assertFailsWith<IllegalArgumentException> { ValidationUtils.validateUserMetadata(metadata) }
    assertTrue(ex.message!!.contains("Metadata ID cannot be empty"))
  }

  @Test
  fun `validateUserMetadata throws for blank userId`() {
    val metadata = UserMetadata(id = "userMeta1", userId = "", timestamp = dummyTimestamp)
    val ex =
        assertFailsWith<IllegalArgumentException> { ValidationUtils.validateUserMetadata(metadata) }
  ...
  fun `validateHeartRate throws for zero bpm`() {
    val ex = assertFailsWith<IllegalArgumentException> { ValidationUtils.validateHeartRate(0) }
    assertTrue(ex.message!!.contains("Heart rate (BPM) must be positive"))
  }

  @Test
  fun `validateHeartRate throws for unrealistically high bpm`() {
    val ex = assertFailsWith<IllegalArgumentException> { ValidationUtils.validateHeartRate(500) }
  ...
  fun `validateCalories throws for negative value`() {
    val ex = assertFailsWith<IllegalArgumentException> { ValidationUtils.validateCalories(-10.0) }
    assertTrue(ex.message!!.contains("Calories must be non-negative"))
  }

  @Test
  fun `validateExerciseId throws for blank`() {
    val ex = assertFailsWith<IllegalArgumentException> { ValidationUtils.validateExerciseId("") }
    assertTrue(ex.message!!.contains("Exercise ID cannot be empty"))
  }

  @Test
  fun `validateUserId throws for blank`() {
    val ex = assertFailsWith<IllegalArgumentException> { ValidationUtils.validateUserId("") }
    assertTrue(ex.message!!.contains("User ID cannot be empty"))
  }
}
